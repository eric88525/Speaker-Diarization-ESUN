...
class ModelTrainer(object):
    ...
    # used for SD
    # input 1 dim wav return embedding
    def SD_test(self, emb):
        self.__model__.eval();
        ref_feat = self.__model__(emb).detach().cpu()
    ...
    def evaluateFromList(self, test_list, test_path, nDataLoaderThread, print_interval=100, num_eval=10, **kwargs):
        ...
        for idx, line in enumerate(lines):
            ...
            score2 = criterion.forward(ref_feat, com_feat).data.cpu().numpy()
            #print('ans', data[0], score2)
    ...
    def CosevaluateFromList(self, test_list, test_path, nDataLoaderThread, print_interval=100, num_eval=10, **kwargs):
        ...
        for idx, data in enumerate(test_loader):
            ...
            #print(type(inp1), data[0][0].shape)
            if idx % print_interval == 0:
                sys.stdout.write("\rReading %d of %d: %.2f Hz, embedding size %d"%(idx,len(setfiles),idx/telapsed,ref_feat.size()[1]));
        ...
        for idx, line in enumerate(lines):
            ...
            criterion = nn.CosineSimilarity(dim=1, eps=1e-6)
            score = criterion.forward(ref_feat, com_feat).data.cpu().numpy()
            #print('ans', data[0], score2)
        ...
        return (numpy.array(all_scores), numpy.array(all_labels), all_trials);


    def PevaluateFromList(self, test_list, test_path, nDataLoaderThread, print_interval=100, num_eval=10, **kwargs):
